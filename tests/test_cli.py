...
def test_cluster_relabel_reuses_cached_clustering_but_recomputes_labels(tmp_path, monkeypatch):
    monkeypatch.setattr(cli_module.settings, "chroma_dir", tmp_path / "chroma")
    monkeypatch.setattr(cli_module.settings, "output_dir", tmp_path / "output")
    collection = get_collection(cli_module.settings)
    n = 12  # UMAP needs enough points to build a manifold graph — 2 points degenerates.
    ...
    runner = CliRunner()
    first = runner.invoke(cli, ["cluster"])
    assert first.exit_code == 0, first.output

    monkeypatch.setattr(cli_module, "cluster_items", MagicMock(side_effect=AssertionError("recomputed")))

    def fake_label_clusters(items, settings):
        for item in items:
            item.cluster_label = "Relabeled"
        return items

    monkeypatch.setattr(cli_module, "label_clusters", fake_label_clusters)

    second = runner.invoke(cli, ["cluster", "--relabel"])

    assert second.exit_code == 0, second.output
    cache = cli_module.load_cluster_cache(cli_module.settings)
    assert all(item.cluster_label == "Relabeled" for item in cache if item.cluster_id != -1)
